-- Remove campo "Idade que irá completar" — será calculado automaticamente depois.

delete from public.tenant_closing_form_fields
where field_key = 'aniversariante_idade';
...
begin
  ...
  values
    ...
    (new.id, 'aniversariante', 'Tema da festa', 'aniversariante_tema', 'text', true, true, 3, true, 'experiencia', false, false, true, true, false),
    (new.id, 'aniversariante', 'Personagem favorito', 'aniversariante_personagem', 'text', false, true, 4, true, 'experiencia', false, false, true, true, false),
...
set sort_order = case field_key
  when 'aniversariante_tema' then 3
  when 'aniversariante_personagem' then 4
  else sort_order
end
where field_key in ('aniversariante_tema', 'aniversariante_personagem');
