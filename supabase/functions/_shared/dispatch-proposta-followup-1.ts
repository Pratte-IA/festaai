...
import {
  buildPropostaFollowup1Message,
  buildPropostaFollowup1Nota,
} from "./proposta-followup-1-message.ts";
import {
  PROPOSTA_FOLLOWUP_1_EVENT,
...
export interface DispatchPropostaFollowup1Input {
  eventoId: number;
  tenant: {
    id: number;
    name: string;
...
export interface DispatchPropostaFollowup1Result {
  dispatched: boolean;
  errorMessage: string | null;
  responseStatus: number | null;
  skippedReason: string | null;
  variante: PropostaFollowup1Variante | null;
}
...
export const dispatchPropostaFollowup1 = async (
  admin: ServiceClient,
  ...
  const { data: evento, error: eventoError } = await admin
    .from("eventos")
    .select(
      "id, aniversariante_nome, cliente_nome, cliente_telefone, data_evento, etapa, followup_1_enviado_em, followup_status, funil",
    )
    .eq("tenant_id", input.tenant.id)
  ...
  if (!evento) {
    return {
      dispatched: false,
      errorMessage: "Evento não encontrado para follow-up 1.",
      responseStatus: null,
      skippedReason: null,
      variante: null,
    };
  ...
  if (evento.followup_1_enviado_em) {
    return {
      ...
      errorMessage: null,
      responseStatus: null,
      skippedReason: "Follow-up 1 já enviado para este evento.",
      variante: null,
    };
  }
  ...
  await admin.from("automation_dispatch_logs").insert({
    ...
    event: PROPOSTA_FOLLOWUP_1_EVENT,
  ...
  if (sendResult.ok) {
    await admin
      .from("eventos")
      .update({
        followup_1_enviado_em: input.triggeredAt,
        followup_1_variante: variante,
      })
      .eq("tenant_id", input.tenant.id)
      .eq("id", input.eventoId)
    ...
    await admin.from("evento_notas").insert({
      ...
      tenant_id: input.tenant.id,
      texto: buildPropostaFollowup1Nota({
        dataEvento,
